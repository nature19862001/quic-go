...
func newServerTLS(
	...
	cs, err := mint.NewDefaultCookieSource()
	if err != nil {
		return nil, nil, err
	}
	mconf.CookieSource = cs
...
func (s *serverTLS) handleInitialImpl(remoteAddr net.Addr, hdr *wire.Header, data []byte) (packetHandler, error) {
	...
	if !protocol.IsSupportedVersion(s.supportedVersions, hdr.Version) {
		...
		_, err := s.conn.WriteTo(wire.ComposeVersionNegotiation(hdr.ConnectionID, hdr.PacketNumber, hdr.Version, s.supportedVersions), remoteAddr)
		return nil, err
	}
